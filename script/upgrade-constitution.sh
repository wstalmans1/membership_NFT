#!/bin/bash
# Script to upgrade Constitution proxy using OpenZeppelin Foundry Upgrades plugin
...
echo -e "${BLUE}=== Qawl2 Constitution Upgrade ===${NC}\n"
...
echo -e "${YELLOW}This will upgrade the Constitution proxy on Sepolia${NC}"
...
echo "Next steps:"
echo "  1. Call migrateAllowedRecipients() to populate the enumerable list"
echo "     Use: forge script script/MigrateAllowedRecipients.s.sol:MigrateAllowedRecipients --rpc-url \$SEPOLIA_RPC_URL --broadcast -vvvv"
echo "  2. Verify the new implementation address on Blockscout"
echo "  3. Update CONTRACT_ADDRESSES.md with the new implementation address"
echo ""
echo "View Constitution proxy on Blockscout:"
echo "  https://eth-sepolia.blockscout.com/address/0x931E702cfdda455f77dAdD55F79D561588FbcD05"
